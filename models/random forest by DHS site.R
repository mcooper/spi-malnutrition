...
hh$residuals <- lm(haz_dhs ~ age + as.factor(calc_birthmonth) + 
                  birth_order + hhsize + sex + mother_years_ed + toilet +
                  head_age + head_sex + wealth_index, data=hh) %>%
  residuals

hh <- hh %>%
  group_by(code, interview_year) %>%
  summarize(residual=mean(residuals))

all <- Reduce(function(x, y){merge(x,y,all.x=T, all.y=F)}, list(hh, spei, cov)) %>%
  na.omit
...
data <- all %>%
  dplyr::select(spei24, ag_pct_gdp, precip_10yr_mean, 
...
residuals <- all$residual

rfmod <- randomForest(residuals ~ ., data=data, ntree=400)
