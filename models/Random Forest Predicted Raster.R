...
all$residuals <- lm(haz_dhs ~ age + as.factor(calc_birthmonth) + 
                      birth_order + hhsize + sex + mother_years_ed + toilet +
                      head_age + head_sex + wealth_index, data=all) %>%
  residuals

randomForest(residuals ~ spei24 + bare + precip_10yr_mean +
             ...
             builtup + elevation + high_settle + low_settle + roughness, data=all)
