...
all <- all %>%
  select(-interview_month, -interview_cmc, -calc_birthmonth, -calc_birthyear, -thousandday_month, -thousandday_year, 
         ...
         #-spi6, -spi12, -spi36, 
         -birthday_9monthtotal, -birthday_spei9,
...
all$spei24sq <- all$spei24^2
all$surveycode <- substr(all$code, 1, 6)

all <- all %>%
  filter(!is.infinite(spei24) & !is.infinite(spei12) & !is.infinite(spei6) & !is.infinite(spei36))

all$drought <- ifelse(all$spei12 > 1 & all$spei12 < 1.5, "Wet",
                      ifelse(all$spei12 > 1.5, "Very Wet", 
                             ifelse(all$spei12 < -1 & all$spei12 >-1.5, "Dry",
                                    ifelse(all$spei12 < -1.5, "Very Dry", "Normal")))) %>%
  factor(levels=c("Normal", "Wet", "Dry", "Very Wet", "Very Dry"))


lowspimod <- lm(haz ~ age + interview_year + head_sex + hhsize + sex + gdp + pop + mean_annual_precip +
                 head_age + md + wealth_index + mother_years_ed + workers + related_hhhead + spei12 + 
                 istwin + diarrhea + fever + country, 
               data = all[all$spei12 < 0, ])
summary(lowspimod)

highspimod <- lm(haz ~ age + interview_year + head_sex + hhsize + sex + gdp + pop + mean_annual_precip +
               head_age + md + wealth_index + mother_years_ed + workers + related_hhhead +spei12 + 
               istwin + diarrhea + fever + country, 
             data = all[all$spei12 > 0, ])
summary(highspimod)
