...
all <- Reduce(function(x, y){merge(x, y, all.x=T, all.y=F)},
              list(hh, gdp, farm, lc, md, pop, spei, regions))
...
all <- all %>%
  filter(!is.na(all$spei12) & !is.infinite(all$spei12))

library(mgcv)

all$interview_year <- as.factor(all$interview_year)

test <- gamm(haz ~ s(spei12, bs="cr") + age + head_sex + hhsize + sex + gdp + pop +
              head_age + md + wealth_index + mother_years_ed + workers + related_hhhead + 
              istwin + diarrhea + fever, data=all, random=list(country=~1,interview_year=~1))

testdata = data.frame(age = mean(all$age, na.rm=T),
                      interview_year = '2007',
                      head_sex = 'Male',
...
fits = predict(test, testdata, type='response', se=T)
predicts = data.frame(testdata, fits) %>% 
  mutate(lower = fit - 1.96*se.fit,
         upper = fit + 1.96*se.fit)

ggplot(aes(x=spei12,y=fit), data=predicts) +
