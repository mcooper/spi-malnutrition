...
all <- all %>%
  filter(!is.na(all$spi24) & !is.infinite(all$spi24))

library(mgcv)


test <- gam(haz ~ s(spi24) + age + interview_year + head_sex + hhsize + sex + gdp + pop +
...
testdata = data.frame(age = mean(all$age, na.rm=T),
                      interview_year = mean(all$interview_year, na.rm=T),
                      head_sex = 'Male',
